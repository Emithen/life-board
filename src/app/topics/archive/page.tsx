    ...
    <main className="min-h-screen bg-[#f7f7f4] px-5 py-6 sm:px-8 lg:px-12">
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-8">
        <header className="flex items-end justify-between border-b border-neutral-200 pb-6">
          ...
          <Link href="/" className="inline-flex h-10 items-center gap-1.5 border border-neutral-300 bg-white px-3 text-sm"><ChevronLeft size={16} /> 문서</Link>
        </header>
        ...
        <section className="border border-neutral-200 bg-white">
          {items.length === 0 ? <p className="p-12 text-center text-sm text-neutral-500">보관된 문서가 없습니다.</p> : items.map((topic) => (
